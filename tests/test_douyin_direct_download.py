...
from services.video_validation_service import InvalidVideoFileError
...
class DouyinDirectDownloadTests(unittest.TestCase):
    ...
    def test_active_download_path_never_focuses_or_clicks_browser_ui(self):
        source = "\n".join(
            (
                ...
                inspect.getsource(download_video_via_extension),
            )
        )
    ...
    def test_captured_response_is_used_without_checking_extension(self):
        ...
        result = _wait_for_douyin_bridge_result(
            page,
            "123",
            timeout_seconds=1,
    ...
    @patch(
        "profile_automation.pipeline.profile_worker._try_douyin_extension_result",
        return_value={
            "status": "success",
            "aweme_id": "123",
    ...
    @patch(
        "profile_automation.pipeline.profile_worker._wait_for_douyin_direct_result",
    ...
    def test_extension_is_only_used_after_direct_sources_fail(self, direct_wait, extension_wait):
        result = _wait_for_douyin_bridge_result(FakeBridgePage(), "123", timeout_seconds=2)
    ...
    @patch("profile_automation.pipeline.profile_worker.create_background_page")
    @patch("profile_automation.pipeline.profile_worker.connected_gemlogin_profile")
    ...
    @patch("profile_automation.pipeline.profile_worker.validate_video_file")
    ...
    @patch("profile_automation.pipeline.profile_worker._download_douyin_url")
