...
def _clean_orphan_source_states(base_dir: Path, cutoff: float) -> int:
    state_dir = base_dir / "profile_automation" / "state"
...
def run_runtime_maintenance(
    *,
    ...
    try:
        from services.telegram_service import prune_telegram_caption_state

        summary.update(prune_telegram_caption_state())
    except Exception as exc:
        logger.warning("Không thể dọn trạng thái caption Telegram cũ: %s", exc)
    try:
        from services.stats_service import prune_stats_history

        summary["stats_days"] = prune_stats_history()
    except Exception as exc:
        logger.warning("Không thể dọn thống kê ngày cũ: %s", exc)
    ...
    summary["old_log_backups"] = sum(
        ...
        for log_root in (root / "logs", root / "core" / "logs")
    )
    ...
    logger.info("[Maintenance] Đã hoàn tất xoay dữ liệu runtime: %s", summary)
    return summary
