...
from services.publishing.extension_upload_service import (
    LOCAL_SOURCE_KEY,
    LOCAL_SOURCE_LABEL,
    SUPPORTED_PLATFORMS,
...
class ManualPublishService:
    ...
    def submit(self, request: ManualPublishRequest) -> dict[str, Any]:
        ...
        if len(requested_items) > 20:
            raise ValueError("Mỗi lần chỉ được chọn tối đa 20 video.")
        if not request.targets:
            raise ValueError("Hãy chọn ít nhất một Profile để đăng.")

        normalized_items: list[tuple[Path, str, str]] = []
        for index, item in enumerate(requested_items, start=1):
            ...
            normalized_items.append((resolved_path, caption, scheduled_at))
        ...
        scheduled_count = 0
        for index, (file_path, caption, scheduled_at) in enumerate(normalized_items, start=1):
            video_id = f"local_{batch_id}_{index}"
            source_label = f"{LOCAL_SOURCE_LABEL} · {file_path.name}"
            if scheduled_at:
                scheduled_count += 1
            for target in normalized_targets:
