    ...
    *,
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            last_error = exc
            if not is_gemlogin_connection_error(exc) or attempt >= max_attempts:
                raise
            _LOGGER.warning(
...
@contextmanager
def connected_gemlogin_browser(
    debug_address: str,
    *,
    attempts: int = 3,
    retry_delay_seconds: float = 2,
):
    """Connect through a fresh serialized Playwright driver and keep it alive for the operation."""
    max_attempts = max(1, int(attempts))
    delay = max(0.0, float(retry_delay_seconds))
    ...
    with _PLAYWRIGHT_OPERATION_LOCK, _cross_process_playwright_lock():
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                candidate_manager, candidate_playwright, candidate_browser = (
                    _start_playwright_connection(debug_address)
                )
            except Exception as exc:
                last_error = exc
                ...
                if attempt >= max_attempts:
                    break
                _LOGGER.warning(
                    "Playwright/CDP connection failed on attempt %s/%s; starting a fresh driver: %s",
                    attempt,
                    max_attempts,
                    exc,
                )
        ...
        if browser is None:
            raise RuntimeError(
                f"Không thể kết nối GemLogin qua CDP sau {max_attempts} lần: {last_error}"
            ) from last_error
