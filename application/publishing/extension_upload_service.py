...
from application.workflows.profile_worker import ProfileWorker, get_tracking_download_path
...
LOCAL_SOURCE_KEY = "dyna_local_publish"
LOCAL_SOURCE_LABEL = "Publish Center"
INGEST_SOURCE_KEYS = {EXTENSION_SOURCE_KEY, LOCAL_SOURCE_KEY}
...
class ExtensionUploadRequest:
    ...
    download_url: str = ""
    download_id: int = 0
    source_key: str = EXTENSION_SOURCE_KEY
    source_label: str = EXTENSION_SOURCE_LABEL
    platforms: tuple[str, ...] = ()
    use_supplied_caption: bool = False
    scheduled_at: str = ""
    batch_id: str = ""
    batch_name: str = ""
...
class ExtensionUploadService:
    ...
    def list_profiles(self) -> list[dict[str, Any]]:
        ...
        for path in sorted(self.profiles.profile_dir.glob("profile_*.json")):
            ...
            try:
                ...
                logger.warning("Không thể đọc Profile %s cho tiện ích mở rộng: %s", profile_id, exc)
                continue
            platforms = {
    ...
    @staticmethod
    def _profile_for_platforms(
        profile: dict[str, Any],
        selected_platforms: tuple[str, ...] | list[str],
    ) -> dict[str, Any]:
        requested = tuple(dict.fromkeys(str(item).strip().lower() for item in selected_platforms if str(item).strip()))
        if not requested:
            return profile
        unknown = [item for item in requested if item not in SUPPORTED_PLATFORMS]
        if unknown:
            raise ValueError(f"Nền tảng không được hỗ trợ: {', '.join(unknown)}")
        unavailable = [
            item
            for item in requested
            if not bool((profile.get(item) or {}).get("enabled", False))
        ]
        if unavailable:
            raise ValueError(
                "Profile chưa bật nền tảng: " + ", ".join(unavailable)
            )
        scoped = deepcopy(profile)
        for platform in SUPPORTED_PLATFORMS:
            scoped.setdefault(platform, {})["enabled"] = platform in requested
        return scoped
    ...
    def submit(self, request: ExtensionUploadRequest) -> dict[str, Any]:
        ...
        profile = self._profile_for_platforms(profile, request.platforms)
        ...
        source = {
            "source_key": str(request.source_key or EXTENSION_SOURCE_KEY),
            "target_sec_uid": str(request.author_uid or "extension"),
            "target_display_name": str(request.source_label or EXTENSION_SOURCE_LABEL),
            "enabled": True,
        }
        ...
        if bool(job.get("active")):
            return {"accepted": False, "duplicate": True, "job": job}
        if request.batch_id or request.batch_name:
            self.jobs.set_batch(
                profile_id,
                video_id,
        ...
        if request.use_supplied_caption:
            self.jobs.set_caption(profile_id, video_id, request.description)
            job = self.jobs.get_job(profile_id, video_id) or job
        ...
        with self._lock:
            ...
            try:
                ...
                self.jobs.set_status(
                    ...
                    video_id,
                    "failed_download",
                    error=f"Nhận file video thất bại: {exc}",
                )
                raise RuntimeError(f"Dyna không thể nhận file video: {exc}") from exc
            if request.scheduled_at:
                self.jobs.set_schedule(profile_id, video_id, request.scheduled_at)
            else:
    ...
    def _copy_into_managed_storage(
        ...
        if source_path.suffix.lower() != managed_path.suffix.lower():
            managed_path = managed_path.with_suffix(source_path.suffix.lower())
    ...
    def _start_processing(
        ...
        with self._lock:
            ...
            thread = threading.Thread(
                ...
                daemon=True,
                name=f"local-upload-{profile_id}-{video.aweme_id}",
            )
    ...
    def _process_managed_upload(
        ...
        try:
            ...
            logger.info(
                "[Đăng video local] Đã nhận video %s cho Profile %s từ %s.",
                video.aweme_id,
                profile_id,
            ...
            logger.exception(
                "[Đăng video local] Không thể xử lý video %s cho Profile %s: %s",
                video.aweme_id,
                profile_id,
                exc,
            )
            try:
                self.jobs.set_status(
                    ...
                    video.aweme_id,
                    "failed_upload",
                    error=f"Xử lý video local thất bại: {exc}",
                )
            except Exception:
                pass
        finally:
    ...
    def resume_pending_jobs(self) -> int:
        """Resume imported jobs that Dyna owned before its last shutdown."""
        resumed = 0
        for job in self.jobs.list_jobs(include_terminal=False):
            source_key = str(job.get("source_key") or "")
            if source_key not in INGEST_SOURCE_KEYS:
                continue
            ...
            try:
                profile = self.profiles.load(profile_id)
                profile = self._profile_for_platforms(
                    profile,
                    list(job.get("enabled_platforms") or []),
                )
                ...
                logger.exception(
                    "[Tiện ích] Không thể khôi phục video %s cho Profile %s: %s",
                    job.get("video_id"),
                    profile_id,
                    exc,
                )
    ...
    def start_scheduled_job(self, profile_id: str, video_id: str) -> dict[str, Any] | None:
        """Start a persisted scheduled job when its publish time is due."""
        current = self.jobs.get_job(str(profile_id), str(video_id))
        if current is None or str(current.get("source_key") or "") != LOCAL_SOURCE_KEY:
            return None
        if str(current.get("status") or "") != "scheduled":
            return current
        download_path = Path(str(current.get("download_path") or ""))
        if not download_path.is_file():
            raise FileNotFoundError("Không còn bản sao video do Dyna quản lý cho lịch đăng này.")

        profile = self.profiles.load(str(profile_id))
        profile = self._profile_for_platforms(
            profile,
            list(current.get("enabled_platforms") or []),
        )
        video = DouyinVideo(**dict(current.get("video") or {}))
        source = {
            "source_key": LOCAL_SOURCE_KEY,
            "target_sec_uid": str(video.author_uid or "local"),
            "target_display_name": str(current.get("source_label") or LOCAL_SOURCE_LABEL),
            "enabled": True,
        }
        self.jobs.set_status(str(profile_id), str(video_id), "downloaded")
        self._start_processing(str(profile_id), profile, video, source, download_path)
        return self.jobs.get_job(str(profile_id), str(video_id))

    def retry_imported_job(self, profile_id: str, video_id: str) -> dict[str, Any] | None:
        """Restart a failed imported job, including Publish Center jobs."""
        current = self.jobs.get_job(str(profile_id), str(video_id))
        if current is None or str(current.get("source_key") or "") not in INGEST_SOURCE_KEYS:
            return None
        if not str(current.get("status") or "").startswith("failed"):
            return current
        download_path = Path(str(current.get("download_path") or ""))
        if not download_path.is_file():
            raise FileNotFoundError("Không còn bản sao video do Dyna quản lý để thử lại.")
        ...
        self.jobs.retry_job(str(profile_id), str(video_id))
        self._start_processing(str(profile_id), profile, video, source, download_path)
        return self.jobs.get_job(str(profile_id), str(video_id))
