...
def _save_local(data: dict):
    try:
        ...
        logger.error(f"Không thể lưu license: {e}")
...
def verify_with_server() -> dict:
    ...
    try:
        resp = requests.post(
            f"{API_BASE_URL}/api/license/check",
        ...
        logger.warning("Không kết nối được server để xác thực license")
    except ValueError as e:
        return {"username": uname, "is_active": False, "error": str(e)}
    except Exception as e:
        logger.error(f"Lỗi xác thực license: {e}")
...
def poll_payment_status(order_id: str) -> dict:
    resp = requests.get(
        f"{API_BASE_URL}/api/payment-status/{order_id}",
        timeout=8,
    )
    resp.raise_for_status()
    return resp.json()
