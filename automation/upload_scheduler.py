"""Legacy Google Sheet uploader retained for migration; it is not started by DynaTool."""

import time, gspread, re, os, sys, random
from datetime import datetime, timedelta
from oauth2client.service_account import ServiceAccountCredentials
from rich.console import Console
from rich.rule import Rule
import threading
from services.telegram_service import send_upload_success_notification
...
def _upload_one_video(context, page, sheet, row_idx, row_data, idx_map, profile_id="1"):
    """
    Upload 1 video cụ thể lên TikTok.
    context: playwright browser context
    page: background page created by the shared GemLogin session manager
    ...
    try:
        ...
        set_video_file_background(
            page,
            file_path,
            lambda: page.mouse.click(CLICK_UPLOAD_X, CLICK_UPLOAD_Y),
        )
        logger.info("[Profile %s] Da chon video TikTok bang Playwright o che do nen.", profile_id)
                ...
                try:
                    ...
                    for row_idx, row_data in videos_to_upload:
                        ...
                        def do_upload(prof_id=p_id, r_idx=row_idx, r_data=row_data, sht=sheet, imap=idx_map):
                            try:
                                with connected_gemlogin_profile(prof_id, API_URL) as browser:
                                    if not browser.contexts:
                                        logger.error("Profile %s has no browser context.", prof_id)
                                        return
                                    ...
                                    logger.info("==========================================================")
                                    _upload_one_video(context, page, sht, r_idx, r_data, imap, str(prof_id))
                            except Exception as e:
