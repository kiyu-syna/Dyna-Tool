  ...
  return (
    <div className="page-stack publish-page">
      <div className="publish-intro">
        <FileVideo size={20} />
        <div>
          <strong>{l("Đăng video có sẵn trên máy", "Publish videos from this computer", "发布本机视频")}</strong>
          <span>
            {l(
              "Mỗi video có mô tả và lịch riêng; Dyna giữ nguyên file gốc và lưu lịch qua lần khởi động lại.",
              "Each video has its own caption and schedule; Dyna preserves the source file and schedule across restarts.",
              "每个视频都可设置独立文案和发布时间；Dyna 会保留源文件和排期。",
            )}
          </span>
        </div>
      </div>
      ...
      <Section
        title={`1. ${l("Video, mô tả và lịch đăng", "Videos, captions, and schedules", "视频、文案和排期")} · ${items.length}/20`}
        className="publish-items-section"
        action={
          <button className="small-button" onClick={() => void chooseFiles()}>
            <FolderOpen size={14} />
            {l("Chọn video", "Select videos", "选择视频")}
          </button>
        }
      >
        {!items.length ? (
          <button className="publish-dropzone" onClick={() => void chooseFiles()}>
            <FolderOpen size={25} />
            <strong>{l("Chọn một hoặc nhiều video", "Select one or more videos", "选择一个或多个视频")}</strong>
            <span>
              {l(
                "Sau khi chọn, nhập mô tả và thời gian riêng cho từng video",
                "Then enter a caption and time for each video",
                "选择后，为每个视频填写文案和发布时间",
              )}
            </span>
          </button>
              ...
              {items.map((item, index) => (
                <article key={item.file_path}>
                  <header>
                    ...
                    <FileVideo size={16} />
                    <span>
                      <strong title={item.file_path}>{fileName(item.file_path)}</strong>
                      <small title={directoryName(item.file_path)}>{directoryName(item.file_path)}</small>
                    </span>
                  ...
                  <div className="publish-item-fields">
                    <label>
                      ...
                      <textarea
                        ...
                        rows={3}
                        placeholder={l(
                          `Mô tả cho ${fileName(item.file_path)}...`,
                          `Caption for ${fileName(item.file_path)}...`,
                          `${fileName(item.file_path)} 的文案...`,
                        )}
      ...
      <Section
        title={`${l("Hàng đợi Trung tâm đăng", "Publish Center queue", "发布中心队列")}${jobs.data ? ` · ${jobs.data.total}` : ""}`}
        action={
          <button
